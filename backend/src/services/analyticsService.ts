...
// Union type for all analytics event payloads
export type AnalyticsEventPayload =
  ...
  | ImageAnalytics
  | PerformanceMetrics
  | { [key: string]: any }; // Fallback for custom events
...
export class AnalyticsService {
  ...
  static logImageTrigger(data: {
    ...
    console.log(
      `[ANALYTICS] Image triggered: ${data.imageName} (${
        data.imageId
      }) - Script: "${data.scriptText.substring(0, 50)}..."`,
    );

    if (data.recognitionConfidence) {
      console.log(
        `[ANALYTICS] Recognition confidence: ${(
    ...
    AnalyticsService.persistAnalyticsEvent(
      "image_trigger",
      {
        imageId: data.imageId,
        imageName: data.imageName,
      ...
      console.warn("[ANALYTICS] Failed to persist image trigger event:", err);
    });
  ...
  static logAvatarPlayStart(data: {
    ...
    console.log(
      `[ANALYTICS] Avatar play started: Video ${data.videoId} for image ${data.imageId}`,
    );
    ...
    AnalyticsService.persistAnalyticsEvent(
      ...
      console.warn(
        "[ANALYTICS] Failed to persist avatar play start event:",
        err,
      );
    });
  ...
  static logAvatarPlayEnd(
    ...
    console.log(
      `[ANALYTICS] Avatar play ended: ${eventId} - Duration: ${event.duration}ms, Status: ${status}`,
    );
    ...
    AnalyticsService.persistAnalyticsEvent(
      ...
      console.warn("[ANALYTICS] Failed to persist avatar play end event:", err);
    });
  }
  ...
  static logResponseTime(responseTime: number, success: boolean = true): void {
    ...
    // Best-effort persistence
    AnalyticsService.persistAnalyticsEvent("response_time", {
      responseTime,
      success,
      timestamp: new Date().toISOString(),
    }).catch((err) => {
      console.warn("[ANALYTICS] Failed to persist response time event:", err);
    });
  }
  ...
  static logVideoProcessingTime(processingTime: number): void {
    ...
    if (performanceMetrics.videoProcessingTimes.length > 1000) {
      performanceMetrics.videoProcessingTimes =
        performanceMetrics.videoProcessingTimes.slice(-1000);
    }

    // Best-effort persistence
    AnalyticsService.persistAnalyticsEvent("video_processing_time", {
      processingTime,
      timestamp: new Date().toISOString(),
    }).catch((err) => {
      console.warn(
        "[ANALYTICS] Failed to persist video processing time event:",
        err,
      );
    });
  }
  ...
  static getAnalytics(): {
    ...
    const imageCounts = new Map<
      string,
      { imageId: string; imageName: string; count: number }
    >();
    ...
    imageTriggerEvents.forEach((event) => {
      ...
      const existing = voiceCounts.get(key) || {
    ...
    const completedPlays = Array.from(avatarPlayEvents.values()).filter(
      (event) => event.status === "completed",
    );
    const interruptedPlays = Array.from(avatarPlayEvents.values()).filter(
      (event) => event.status === "interrupted",
    );
    const averageDuration =
      completedPlays.length > 0
        ? completedPlays.reduce(
    ...
    const averageResponseTime =
      performanceMetrics.responseTimes.length > 0
        ? performanceMetrics.responseTimes.reduce(
            (sum, time) => sum + time,
            0,
          ) / performanceMetrics.responseTimes.length
        : 0;

    const averageVideoProcessingTime =
      performanceMetrics.videoProcessingTimes.length > 0
        ? performanceMetrics.videoProcessingTimes.reduce(
            (sum, time) => sum + time,
            0,
          ) / performanceMetrics.videoProcessingTimes.length
        : 0;
    ...
    return {
      imageTriggers: {
        ...
        byImage: Array.from(imageCounts.values())
          .sort((a, b) => b.count - a.count)
          .slice(0, 10),
        byVoice: Array.from(voiceCounts.values())
          .sort((a, b) => b.count - a.count)
          .slice(0, 10),
      },
      avatarPlays: {
      ...
      performance: {
        ...
        mostTriggeredImages: Array.from(imageCounts.values())
          .sort((a, b) => b.count - a.count)
          .slice(0, 5),
        mostUsedVoices: Array.from(voiceCounts.values())
          .sort((a, b) => b.count - a.count)
          .slice(0, 5),
      },
    };
