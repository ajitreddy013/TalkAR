  ...
  secretPatterns.forEach((pattern, index) => {
    ...
    if (matches) {
      matches.forEach(match => {
        addIssue('high', 'Hardcoded Secret', `Found potential hardcoded secret: ${match}`, filePath);
        foundSecrets = true;
      });
    }
  });
...
function checkJWTSecret(secret) {
  if (!secret || secret.length < 32) {
    addIssue('high', 'JWT Security', 'JWT secret is too short (should be at least 32 characters)');
    return false;
  }
  
  if (secret === 'your-super-secret-jwt-key' || secret.includes('example') || secret.includes('demo')) {
    addIssue('high', 'JWT Security', 'JWT secret appears to be a default/demo value');
    return false;
  }
...
function checkSQLInjectionVulnerabilities(content, filePath) {
  ...
  dangerousPatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        addIssue('high', 'SQL Injection', `Potential SQL injection vulnerability: ${match}`, filePath);
...
function checkXSSVulnerabilities(content, filePath) {
  ...
  dangerousPatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        addIssue('medium', 'XSS Vulnerability', `Potential XSS vulnerability: ${match}`, filePath);
        foundVulnerabilities = true;
      });
    }
  });
...
function checkCORSConfiguration(content, filePath) {
  ...
  corsPatterns.forEach(pattern => {
    ...
    if (matches) {
      matches.forEach(match => {
        addIssue('medium', 'CORS Configuration', `Permissive CORS configuration found: ${match}`, filePath);
        foundIssues = true;
      });
    }
  });
...
function checkDependencies(packageJsonPath) {
  try {
    ...
    const knownVulnerabilities = [
      'lodash', 'jquery', 'bootstrap', 'moment', 'request'
    ];
    
    Object.keys(dependencies).forEach(dep => {
      if (knownVulnerabilities.includes(dep)) {
        addIssue('medium', 'Dependencies', `Potentially outdated/vulnerable dependency: ${dep}`, packageJsonPath);
      }
    });
    
    return true;
  } catch (error) {
    addIssue('low', 'Dependencies', `Could not check dependencies: ${error.message}`, packageJsonPath);
    return false;
  }
}
...
function checkEnvironmentConfig() {
  ...
  envFiles.forEach(filePath => {
    if (fs.existsSync(filePath)) {
      ...
      if (content.includes('your-') || content.includes('example') || content.includes('demo')) {
        addIssue('medium', 'Environment Configuration', 'Environment file contains default/example values', filePath);
      }
...
function checkDockerSecurity() {
  const dockerFiles = ['Dockerfile', 'backend/Dockerfile', 'admin-dashboard/Dockerfile'];
  ...
  dockerFiles.forEach(filePath => {
    if (fs.existsSync(filePath)) {
      ...
      if (!content.includes('USER')) {
        addIssue('medium', 'Docker Security', 'Dockerfile does not specify non-root user', filePath);
      }
      
      // Check for latest tag
      if (content.includes(':latest')) {
        addIssue('low', 'Docker Security', 'Using latest tag - consider pinning to specific version', filePath);
      }
      
      // Check for ADD instead of COPY
      if (content.includes('ADD')) {
        addIssue('low', 'Docker Security', 'Using ADD command - consider using COPY instead', filePath);
      }
    }
  });
}
...
function checkSupabaseConfig() {
  const configFiles = ['docker-compose.yml', 'backend/.env.example', 'admin-dashboard/.env.example'];
  ...
  configFiles.forEach(filePath => {
    if (fs.existsSync(filePath)) {
      ...
      if (content.includes('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9')) {
        addIssue('high', 'Supabase Configuration', 'Default Supabase keys detected - generate new keys for production', filePath);
      }
      
      // Check for localhost Supabase URL in production context
      if (content.includes('localhost:8000') && !filePath.includes('.example')) {
        addIssue('medium', 'Supabase Configuration', 'Localhost Supabase URL - configure production URL', filePath);
      }
    }
  });
}
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        if (mode > parseInt('644', 8)) {
          addIssue('medium', 'File Permissions', `File has overly permissive permissions: ${mode.toString(8)}`, filePath);
        }
      } catch (error) {
        addIssue('low', 'File Permissions', `Could not check file permissions: ${error.message}`, filePath);
      }
    }
  });
}
...
async function runSecurityAudit() {
  ...
  const codeFiles = [
    ...getFilesByExtension('backend/src', '.js'),
    ...getFilesByExtension('backend/src', '.ts'),
    ...getFilesByExtension('admin-dashboard/src', '.js'),
    ...getFilesByExtension('admin-dashboard/src', '.ts'),
    ...getFilesByExtension('admin-dashboard/src', '.tsx')
  ];
  ...
  codeFiles.forEach(filePath => {
    try {
      ...
      addIssue('low', 'File Access', `Could not read file: ${error.message}`, filePath);
    }
  });
  ...
  const packageFiles = ['package.json', 'backend/package.json', 'admin-dashboard/package.json'];
...
function generateAuditReport() {
  ...
  const highIssues = auditResults.issues.filter(i => i.severity === 'high').length;
  const mediumIssues = auditResults.issues.filter(i => i.severity === 'medium').length;
  const lowIssues = auditResults.issues.filter(i => i.severity === 'low').length;
  
  log(`Total Issues Found: ${totalIssues}`, colors.cyan);
  log(`High Severity: ${highIssues}`, colors.red);
  log(`Medium Severity: ${mediumIssues}`, colors.yellow);
  log(`Low Severity: ${lowIssues}`, colors.green);
  
  // Risk assessment
  log('\n🎯 RISK ASSESSMENT', colors.bright);
  if (highIssues > 0) {
    log('🔴 HIGH RISK: Critical security issues detected - immediate action required', colors.red);
  ...
  fs.writeFileSync(reportPath, JSON.stringify({
    summary: {
      ...
      mediumIssues,
      lowIssues,
      riskLevel: highIssues > 0 ? 'HIGH' : mediumIssues > 5 ? 'MEDIUM' : 'LOW'
    },
    issues: auditResults.issues,
    timestamp: new Date().toISOString(),
    recommendations: generateRecommendations(highIssues, mediumIssues, lowIssues)
  }, null, 2));
  ...
  if (mediumIssues > 0) {
    ...
    log('6. Update dependencies with known vulnerabilities', colors.yellow);
...
function generateRecommendations(high, medium, low) {
  ...
  if (high > 0) {
    recommendations.push('Address all high severity security issues before production deployment');
    recommendations.push('Replace all hardcoded secrets with environment variables');
